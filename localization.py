...
def find_treasure(car_pos, angle, tag_set, treasures):
    main = "./estimate3d"
    f = os.popen(main)
    time.sleep(1.0)
    f.close()
    ...
    for txz in tag_xz:
        if txz[0] in tag_set:
            ...
            tag_set.add(txz[0])
            treasures[txz[0]] = treasure_pos(car_pos, angle, txz)


def treasure_pos(car_pos, angle, txz):
    x, z = txz[1], txz[2]
...
if __name__ == "__main__":
    ...
    boundary_corners = []
    for i in range(4):
        position = input("please input position of boundary corners "+str(i+1)+"(clockwise), format: x,y : ").split(",")
    ...
    robot = Robot.Robot(left_trim=-9, right_trim=0)
    angle = 0
